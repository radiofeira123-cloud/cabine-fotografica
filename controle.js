...
async function entrarTelaCheia() {
    try {
        ...
        // Atualizar texto do botão para ser mais convidativo após fullscreen
        try {
            const btnIniciar = document.querySelector('#telaSessao .btn-principal') || document.querySelector('button[onclick=\"iniciarSessao()\"]');
            if(btnIniciar) btnIniciar.textContent = '🔵 Clique aqui para iniciar';
        } catch(e){}
...
window.addEventListener('load', () => {
    ...
    // Se já estiver em tela cheia (ex: reload), atualizar texto do botão iniciar
    try{ const btnIniciar = document.querySelector('#telaSessao .btn-principal') || document.querySelector('button[onclick="iniciarSessao()"]'); if(btnIniciar) btnIniciar.textContent = (document.fullscreenElement? '🔵 Clique aqui para iniciar' : btnIniciar.textContent); }catch(e){}
});
